    ...
    <div className="pt-32 pb-24 min-h-screen">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <motion.p
          ...
          className="text-[#F5F0EB]/60 font-[family-name:var(--font-inter)] text-sm leading-relaxed mb-12 max-w-xl"
        >
          For investors and prospective clients looking to set up a sit-down. Share a bit about
          what you&apos;d like to discuss and suggest a few times that work for you.
        </motion.p>
        ...
        {step === "success" ? (
          ...
          >
            ...
            <p className="text-sm text-[#F5F0EB]/50 font-[family-name:var(--font-inter)] max-w-sm mx-auto">
              Thank you — we&apos;ll confirm a time and follow up shortly.
            </p>
          </motion.div>
        ) : (
          <motion.form
          ...
          >
            ...
            {/* Nature of meeting */}
            <div>
              <label className={labelClass}>Nature of meeting *</label>
              <textarea
                required
                rows={4}
                value={formData.natureOfMeeting}
                onChange={(e) => update("natureOfMeeting", e.target.value)}
                placeholder="Investment discussion, partnership, client consultation, etc."
            ...
            {/* Suggested times */}
            <div>
              <p className={labelClass}>Suggest up to three times *</p>
              <div className="space-y-4">
                {([1, 2, 3] as const).map((n) => (
                  <div key={n} className="grid sm:grid-cols-[auto_1fr_1fr] gap-3 sm:items-center">
                    <span className="text-xs text-[#F5F0EB]/30 font-[family-name:var(--font-inter)] uppercase tracking-[0.1em]">
                      Option {n}
                    </span>
                    <input
                      type="date"
                      required={n === 1}
                      value={formData[`option${n}Date` as const]}
                      onChange={(e) => update(`option${n}Date`, e.target.value)}
                      className={`${inputClass} [color-scheme:dark]`}
                    />
                    <input
                      type="time"
                      required={n === 1}
                      value={formData[`option${n}Time` as const]}
                      onChange={(e) => update(`option${n}Time`, e.target.value)}
                      className={`${inputClass} [color-scheme:dark]`}
                    />
                  </div>
                ))}
            ...
            >
              {step === "loading" ? "Sending…" : "Request Meeting"}
            </button>
          </motion.form>
        )}
      </div>
